...
export default function useHomePageLogic() {
    ...
    const currentSourceMeta = ref({
        source_type: 'url', source_label: '', url: '', file_name: '', article_title: '',
    })

    // ★ 真实今日计数：从 localStorage 读取当日数值
    const todayCount = ref(parseInt(localStorage.getItem(todayKey()) || '0', 10))
    ...
    function onReset(cb) { _resetCallbacks.push(cb) }

    const achievements = ref([
        { id: 1, icon: '⚡', name: '审校先锋', color: 'amber', unlocked: false, goal: '首次审计' },
        { id: 2, icon: '🔥', name: '连续 7 天', color: 'green', unlocked: false, goal: '完成 3 篇' },
        { id: 3, icon: '🏆', name: '审校达人', color: 'blue', unlocked: false, goal: '完成 5 篇' },
        { id: 4, icon: '💎', name: '精英审校', color: 'gray', unlocked: false, goal: '完成 10 篇' },
    ])
    ...
    const tabLabel = computed(() =>
        ({ submit: '新建审计', result: '审计结果', history: '历史记录' })[tab.value] || '',
    )

    const statusLabel = computed(() =>
        ({ running: '审计中…', completed: '已完成', failed: '失败' })[status.value] || '待开始',
    )

    const pillClass = computed(() =>
        ({ running: 'pill-run', completed: 'pill-ok', failed: 'pill-err' })[status.value] || 'pill-idle',
    )
    ...
    const issuesForList = computed(() =>
        exportMode.value ? displayIssues.value.filter(i => !isRejected(i.id)) : displayIssues.value,
    )
    ...
    const allResolved = computed(() =>
        displayIssues.value.length > 0 && displayIssues.value.every(i => isRejected(i.id)),
    )
    ...
    const motSub = computed(() => {
        ...
        return next ? `再完成更多篇可解锁「${next.name}」成就。` : '所有成就已解锁，厉害了！'
    })
    ...
    function historyTitle(item) {
        ...
        if (isRealTitle(item.source_label) && !/^https?:\/\//i.test(item.source_label))
    ...
    async function loadKnowledgeBases() {
        const [clResp, refResp] = await Promise.all([apiFetch('/kb/checklists'), apiFetch('/kb/references')])
        if (clResp.ok) { const data = await clResp.json(); checklistKbs.value = Array.isArray(data.items) ? data.items : [] }
        if (refResp.ok) { const data = await refResp.json(); referenceKbs.value = Array.isArray(data.items) ? data.items : [] }
    }

    async function saveChecklistToKb() {
        try {
            const items = parseChecklist(checklistInput.value)
            if (!items.length) { error.value = '清单为空，无法保存'; return }
            const resp = await apiFetch('/kb/checklists', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name: (newChecklistName.value || '未命名清单').trim(), items }),
            })
            if (!resp.ok) { throw new Error(await resp.text() || `保存清单失败：${resp.status}`) }
            const created = await resp.json()
            checklistKbs.value = [created, ...checklistKbs.value]
            selectedChecklistKbIds.value = [created.kb_id, ...selectedChecklistKbIds.value]
            newChecklistName.value = ''
            showToast('清单已保存到知识库')
        } catch (e) { error.value = e?.message || '保存清单失败' }
    }

    async function saveReferencesToKb() {
        try {
            if (!referenceFiles.value.length) { error.value = '请先选择附加资料文件'; return }
            const fd = new FormData()
            for (const f of referenceFiles.value) fd.append('files', f)
            const resp = await apiFetch('/kb/references/upload', { method: 'POST', body: fd })
            if (!resp.ok) throw new Error(await resp.text() || `保存附加资料失败：${resp.status}`)
            const data = await resp.json()
            const created = Array.isArray(data.items) ? data.items : []
            if (created.length) {
                referenceKbs.value = [...created, ...referenceKbs.value]
                selectedReferenceKbIds.value = [...new Set([...created.map(i => i.kb_id), ...selectedReferenceKbIds.value])]
            }
            referenceFiles.value = []
            if (referenceFileInputRef.value) referenceFileInputRef.value.value = ''
            showToast('附加资料已保存到知识库')
        } catch (e) { error.value = e?.message || '保存附加资料失败'; throw e }
    }

    async function renameChecklistKb(item) {
        try {
            const name = window.prompt('请输入新的清单名称', String(item?.name || '').trim())
            if (name == null) return
            const trimmed = name.trim()
            if (!trimmed) { error.value = '名称不能为空'; return }
            ...
            checklistKbs.value = checklistKbs.value.map(v => v.kb_id === updated.kb_id ? updated : v)
            showToast('清单知识库已改名')
        } catch (e) { error.value = e?.message || '清单改名失败' }
    }

    async function deleteChecklistKb(item) {
        try {
            if (!window.confirm(`确认删除清单知识库"${item?.name || ''}"？`)) return
            const resp = await apiFetch(`/kb/checklists/${item.kb_id}`, { method: 'DELETE' })
            if (!resp.ok) throw new Error(`删除失败：${resp.status}`)
            checklistKbs.value = checklistKbs.value.filter(v => v.kb_id !== item.kb_id)
            selectedChecklistKbIds.value = selectedChecklistKbIds.value.filter(id => id !== item.kb_id)
            showToast('清单知识库已删除')
        } catch (e) { error.value = e?.message || '清单删除失败' }
    }

    async function renameReferenceKb(item) {
        try {
            const name = window.prompt('请输入新的资料库名称', String(item?.name || '').trim())
            if (name == null) return
            const trimmed = name.trim()
            if (!trimmed) { error.value = '名称不能为空'; return }
            ...
            if (!resp.ok) throw new Error(`改名失败：${resp.status}`)
            const updated = await resp.json()
            referenceKbs.value = referenceKbs.value.map(v => v.kb_id === updated.kb_id ? updated : v)
            showToast('附加资料知识库已改名')
        } catch (e) { error.value = e?.message || '资料库改名失败' }
    }

    async function deleteReferenceKb(item) {
        try {
            if (!window.confirm(`确认删除资料库"${item?.name || ''}"？`)) return
            const resp = await apiFetch(`/kb/references/${item.kb_id}`, { method: 'DELETE' })
            if (!resp.ok) throw new Error(`删除失败：${resp.status}`)
            referenceKbs.value = referenceKbs.value.filter(v => v.kb_id !== item.kb_id)
            selectedReferenceKbIds.value = selectedReferenceKbIds.value.filter(id => id !== item.kb_id)
            showToast('附加资料知识库已删除')
        } catch (e) { error.value = e?.message || '资料库删除失败' }
    }
    ...
    async function startAudit() {
        ...
        try {
            ...
            if (inputMode.value === 'url') {
                ...
                fd.append('selected_checklist_ids', JSON.stringify(selectedChecklistIds))
                fd.append('selected_reference_ids', JSON.stringify(selectedReferenceIds))
                ...
                fd.append('selected_checklist_ids', JSON.stringify(selectedChecklistIds))
                fd.append('selected_reference_ids', JSON.stringify(selectedReferenceIds))
                resp = await apiFetch('/audit/upload', { method: 'POST', body: fd })
            }
    ...
    async function doPoll(id) {
        try {
            ...
            message.value = data.status === 'completed'
                ? `审计完成，发现 ${data.issues?.length || 0} 个问题！`
                : funMessages[idx]
            upsertHistory(buildHistoryItem(
                id, currentSourceMeta.value, data,
    ...
    function removeCustomIssue(id) {
        ...
        const nd = { ...decisionMap.value }, nn = { ...noteMap.value }
    ...
    function endDraw(e) {
        ...
        if (!isDrawing.value || !drawStart.value || !drawCurrent.value) {
            isDrawing.value = false; drawStart.value = null; drawCurrent.value = null; return
        }
        const x = Math.min(drawStart.value.x, drawCurrent.value.x)
        const y = Math.min(drawStart.value.y, drawCurrent.value.y)
    ...
    async function exportPdf() {
        ...
        try {
            ...
            let rem = ih
            pdf.addImage(img, 'PNG', m, m, iw, ih); rem -= ph - m * 2
    ...
    watch(hoverIssueId, (id) => {
        if (!id) return
        nextTick(() => {
            document.querySelector(`.i-card[data-id="${id}"]`)
    ...
    refreshAchievements()
    loadHistory()
    loadKnowledgeBases().catch(e => { error.value = e?.message || '加载知识库失败' })
